...
GA_LiFePO4_Modbus : Translator
{
    ...
    (CAN_ID a_can_id, byte[] a_can_data, Protocol a_protocol, Action<Message_Result<(CAN_ID, byte[])>> a_callback)
    {
        try
        {
            ...
            (Command_Number)a_cmd switch
            {
                ...
                Command_Number.CMD_FG_UNSEAL_FULL => process_fg_unseal_full(a_protocol, a_can_id, a_can_data, a_callback),
                Command_Number.CMD_REBALANCE_MODE => process_enter_rebalance_mode(a_data, ref a_index, a_protocol, a_can_id, a_can_data, a_callback),
        ...
        (
            ...
            a_recv_data =>
            {
                ...
                try
                {
                    ...
                    const ushort CMD = (ushort)(Command_Number.CMD_RESPONSE | Command_Number.CMD_FG_UNSEAL_FULL);
    ...
    (Protocol a_protocol, CAN_ID a_can_id, byte[] a_can_data, Action<Message_Result<(CAN_ID, byte[])>> a_callback)
    {
        ...
        var a_send_data = Modbus_Master.generate_read(SLAVE_ADDR, Modbus_Master.Func_Code.READ_COILS, 10u, 7);
            ...
            a_recv_data =>
            {
                ...
                try
                {
...

                    {
                        ...
                        Misc.decode_be(out a_packet_data.coils, a_data_slice, ref a_index);
...

                    {
                        ...
                        Misc.encode_le(a_packet_data.coils, a_send_data, ref a_index);
