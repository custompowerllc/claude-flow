...
using Translator = translators.GA_LiFePO4_Modbus;
...
GA_LiFePO4 : Product
{
    ...
    new()
    {
        new()
        {
            id = (uint)Status_Register_ID.COILS,
            display_text = "Coils",
            bit7 = ("RSVD", true),
    ...
    (in ReadOnlySpan<byte> a_data)
    {
        ...
        Misc.decode_le(out byte a_coils, a_data, ref a_index);

        using var a_g1 = String_Builder_Pool.acquire();

        var a_result =
            a_g1.value
            .Append($"Coils: 0x{a_coils:X2}; ")
            .ToString();

        add_stat_register_parsed(status_register_map[(uint)Status_Register_ID.COILS], a_coils);
    ...
    (in ReadOnlySpan<byte> a_data)
    {
        ...
        var a_result =
            (Translator.Command_Number)a_cmd switch
            {
                ...
                Translator.Command_Number.CMD_RESPONSE | Translator.Command_Number.CMD_FG_UNSEAL_FULL => parse_fg_unseal_full(a_data),
    ...
    {
        const ushort CMD = (ushort)Translator.Command_Number.CMD_FG_UNSEAL_FULL;
